...
socket.SOCK_STREAM)      # Alternative (better) syntax
...
print("web server on port is:",serverPort)
...
# Extract the path of the requested object from the
includes
filename = message.split()[1]
print ('File name is: ', filename)
# Because the extracted path of the HTTP request
...
character
          f = open(filename[1:])
          # Store the entire contenet of the requested file in a
temporary buffer
socket
outputdata = f.read()
...
Found\r\n\r\n","UTF-8"))
     connectionSocket.send(bytes("<html><head></head><body><h1>4
04 Not Found</h1></body></html>\r\n","UTF-8"))
          # Close the client connection socket
          connectionSocket.close()
serverSocket.close()
